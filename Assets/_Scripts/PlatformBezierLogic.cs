...
public class PlatformBezierLogic : MonoBehaviour
{
  ...
  /*  public void SetUp(GameObject[] normal, GameObject[] low)
    {
        for (int i = 0; i < normal.Length - 1; i++)
        {
            SetUpHelper(normal[i], normal[i + 1]);
        }

        for (int i = 0; i < low.Length; i++)
        {
            SetUpHelper(normal[i]);
        }
    }*/

    public void SetUpHelper(GameObject leftPlatform, GameObject rightPlatform) //// to set up curves between platforms
    {
   ...
   /* private void SetUpHelper(GameObject platform)
    {
        // Get edge of platforms
        Edge1 = platform.GetComponent<EdgeCollider2D>().points[0];
        Edge2 = platform.GetComponent<EdgeCollider2D>().points[1];

        // Initialize curves
        CurveLtoR = (new GameObject("CurveLeftSide")).AddComponent<BezierCurve>();
        CurveRtoL = (new GameObject("CurveRightSide")).AddComponent<BezierCurve>();
        ...
        // Set positions
        BPoint0.transform.position = platform.transform.position + new Vector3(Edge1.x, Edge1.y + 0.2f, 0);
        BPoint1.transform.position = platform.transform.position + new Vector3(Edge2.x, Edge2.y + 0.2f, 0);
    ...
    } //// to set up curves between low platforms and ground
    */
}
